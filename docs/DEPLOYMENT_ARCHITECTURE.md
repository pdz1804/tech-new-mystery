...
| `ecs-sg` | TCP 8000 from `alb-sg`<br/>TCP 3000 from `alb-sg`<br/>TCP 8080 from `ecs-sg` | All traffic | All ECS tasks |
...
| `worker` | `tech-news-prod-worker` | `{ECR}/tech-news-prod-backend:latest` | — | 1024 | 2048 MB | Celery ping |
...
| Total tables | 18 |
| PITR | Enabled on articles · users · clusters tables |
...
| Browser | `bedrock-agentcore:browser` | `tech_news_mystery_prod_browser-Nge15Bp0p9` |
| Code Interpreter | `bedrock-agentcore:code-interpreter` | Set via `CODE_INTERPRETER_ID` env var |

### 4.5 Secrets Manager

**Secret name:** `tech-news-mystery-app-secret`
...
graph LR
    ...
    subgraph AppPipeline[".github/workflows/deploy.yml"]
        ...
        BUILD3["docker build\nagent-core:latest"]
        PUSH["ECR push\n3 repositories"]
        DEPLOY["ECS force-new-deployment\napi · frontend · worker · beat · agent-core"]
