...
### Glassmorphism (Apple Liquid Glass Design)

**Design Philosophy:** Premium glass with extreme transparency, high blur, and subtle glossy highlights instead of solid colors.

```css
/* Hero Section - Maximum Glass Effect */
.hero-glass-lens {
  background: rgba(12, 14, 18, 0.08);           /* 8% opacity - highly transparent */
  backdrop-filter: blur(38px) saturate(180%);   /* Strong frosted glass blur */
  -webkit-backdrop-filter: blur(38px) saturate(180%);
  border: 1px solid rgba(255, 255, 255, 0.24);
  border-top: 1px solid rgba(255, 255, 255, 0.52); /* Glossy top edge highlight */
  box-shadow: 0 24px 56px rgba(0, 0, 0, 0.12), inset 0 1px 0 rgba(255, 255, 255, 0.42);
  border-radius: 28px;
}

/* Standard Glass Panels */
.glass-panel {
  background: rgba(255, 255, 255, 0.8);        /* 80% opacity - clean and readable */
  backdrop-filter: blur(32px) saturate(150%);  /* Professional frosted look */
...
.glass-panel:hover {
  ...
  box-shadow: 0 20px 44px rgba(0, 0, 0, 0.12), inset 0 1px 0 rgba(255, 255, 255, 0.5);
}

/* Article Cards */
.article-glass-card {
  background: rgba(255, 255, 255, 0.72);       /* Slightly more opaque for content */
  backdrop-filter: blur(32px) saturate(150%);
  border: 1px solid rgba(0, 0, 0, 0.08);
  border-top-color: rgba(255, 255, 255, 0.82); /* Glossy edge */
  box-shadow: 0 18px 44px rgba(0, 0, 0, 0.08), inset 0 1px 0 rgba(255, 255, 255, 0.62);
  border-radius: 28px;
}
```

**Key Principles:**
- **Transparency**: 5-25% opacity fill (vs. solid colors) - allows background to show through
- **Blur**: 18-38px backdrop-filter blur for frosted glass effect
- **Glossy Highlights**: 1px top border with white-to-transparent gradient for light catch
- **Subtle Shadows**: Soft shadows for depth without heavy darkness
- **Smooth Transitions**: 300-400ms ease-out for interactive elements

**When to Use:**
- **Hero sections** (maximum transparency): 8% opacity, 38px blur
- **Cards & panels** (balanced readability): 72-80% opacity, 32px blur  
- **Modals & overlays** (content-focused): 80-95% opacity, 32-48px blur
